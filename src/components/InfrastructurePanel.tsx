...
export default function InfrastructurePanel({ parentNode, onClose, onSelectChild }: Props) {
  const { children, loading } = useChildNodes(parentNode?.id || null);

  if (!parentNode) return null;

  // Group children by infrastructure level
  ...
  const levels = ['feeder', 'transformer', 'service_area', 'pole'].filter(l => grouped[l]?.length > 0);

  return (
    <Card className="absolute left-4 top-20 z-[1100] w-80 glass-card shadow-2xl shadow-black/50">
              ...
              return (
                <div key={level} className="space-y-1.5">
                  <div className="flex items-center gap-2 px-2 py-1">
                    ...
                    <Badge variant="secondary" className="ml-auto text-xs">{nodes.length}</Badge>
                  </div>
                    ...
                    {nodes.slice(0, 10).map(node => (
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          {node.feeder_name && (
                            <p className="text-[10px] text-muted-foreground truncate">{node.feeder_name}</p>
                          )}
                        </div>
                        <div className="flex items-center gap-2 ml-2">
                          <div className={`w-1.5 h-1.5 rounded-full ${
                            node.status === 'POWER_AVAILABLE' ? 'bg-success' :
                            node.status === 'OUTAGE' ? 'bg-destructive' : 'bg-warning'
                          }`} />
